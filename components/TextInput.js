    ...
    render(){
        return (
            <div className="input" attr-disabled={(this.props.disabled ? "true" : "false")} attr-validated={(this.props.validated ? "true" : "false")}>
                ...
                `}</style>
                <input type="text" id={'textInput-'+this.props.name} name={this.props.name} placeholder=" " value={this.props.value} onChange={this.handleChange} onFocus={this.handleFocus} disabled={(this.props.disabled ? true : null)}/>
                <label className="label" htmlFor={'textInput-'+this.props.name}>{this.props.placeholder}</label>
                <div className="validatationMessage">{this.props.validationMessage}</div>
            </div>
        );
    }
}
TextInput.defaultProps = {width: '200px', margin:"0 30px 0 0", disabled:false, validated: true, validationMessage:""}
TextInput.propTypes = {
